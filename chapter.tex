...
the order and smoothly put down the tray on the table.}
...
manipulation problems}: several agents have a joint goal that requires some
sort of cooperation to be successfully achieved. These problems are often iterative
(questions-answers, possibly bi-directional) and involve both dialogue and
manipulation.
...
This chapter focuses on the point {\it 2)}: it presents techniques, developed
and used on several real robots, for the creation of a set of environment
models suitable for decision-making and control.

The other items are however equally important to achieve a rich and legible
robot behaviour. Thus it seems important, before really focusing on the
grounding issues, to briefly overview the global architecture of a robot able
to interact with humans\fxwarning{Shall we stay very broad or focus on *our*
implementation?}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Grounding Within a Larger Architecture}
...
The figure~\ref{fig|archi} gives an overview of the overall decisional layer of
...
The dialogue processing module, as well as the symbolic task planner, use also
the knowledge base to answer questions or initialize the planning domain.

The robot controller decides upon a plan to execute, requires a plan from the
task planner, affects the actions to the human and the robot, communicates the
shared plan, and controls and monitors its execution. The operation continues
until the goal is achieved, is declared unachievable or is abandoned by the
human.
...
This means that we assume that all relevant entities (\ie the objects
the robot has to interact with) can be actually perceived by the robot when
they enter its field of view (we shall come back and precisely define the
\emph{field of view} in the next section), that the entities' percepts enable a
\emph{good enough} 6D localization of the entities, that these entities are
properly resolved (\eg, two bottles next to each other are perceived as two
different bottles), and finally that these entities are robustly identified:
each time the exact same object is seen (possibly under a different angle), it
is always labelled with the same, unique, identifier. This identifier by itself
may be however completely random.

Concretly this assumption typically translates into experiments by tagging each
object with a barcode-like label (as seen in Fig.~\ref{fig|vpt}) that allows
the robot to localize and identify a set of objects.

Numerous works \fxfatal{Cite some of them} (some of them presented earlier in
this book) focus on dealing with these resolusion and identification issues.
...
This chapter walks through the creation and use of a symbolic model of the
environment and of the interacting agents, from a 3D geometric model build at
runtime. As far as possible, we will try to present the exact list of symbolic
facts computed by the system at runtime.

We start by introducing the general approach when applied to the symbolic model
building of the environment. We shall introduce as well some \emph{search}
policies and \emph{exploration} policies.

The next section details how the robot create symbolic models that take into
account agents. This section shows that introducing \emph{Perspective
Taking}~\cite{Flavell1992,Tversky1999} and some elements of \emph{Theory of
Mind}~\cite{Scassellati2002}\fxwarning{Check this reference is relevant here}
is mandatory to achieve effective human-robot interaction grounding. Techniques
to efficiently compute perspective-aware models of the world are presented. 

We also introduce in the section~\ref{grounding_agents} an analysis of the
potential of actions (we call it the \emph{mightabilities}) of each agent.
...
applications of these grounding mechanisms. First, we show how the robot may
...
models, close enough from human natural language.

Note that bibliographical notes have been added at the end of each relevant section.
We also present before the conclusion of the chapter several other global architectures
dedicated to the grounding of human-robot interaction, along with some references regarding the specifics of the human-robot interactions themselves.
...
\begin{table}[h]
    \centering
    \begin{tabular}{p{1.5cm}p{5cm}p{2cm}l}
	\rowcolor{white}
    ...
    \hline
	 ...
	 \concept{Location}  & \concept{isAbove} $\equiv$ \concept{cyc:above-Generally}  &  \concept{Location}  &  inverse of \concept{isBelow} \\ 
	...
	equivalent predicate in the {\sc OpenCyc} standard has been added.}
...
\begin{table}[h]
   ...
   \begin{tabular}{p{5cm}p{5cm}l}
	...
	\hline
	 \concept{SpatialThing}  & \concept{isMoving}  &  \concept{xsd:boolean}  
	\end{tabular}
   \caption{Description of agent independent world dynamics.}
   \label{facts|dynamics}
\end{table}
...
Spatial reasoning~\cite{OKeefe1999} has been used
...
In order to correctly interpret the other agents' messages, the robt must be
able to build a model of the agents' state and capabilities and, for each
agent, a model of the environment as perceived by this agent.

To build such an agent-dependent model of the world, \emph{Perspective
...
Table~\ref{facts|capabilites} summarizes the predicates produced by SPARK
...
have already been built based on these theories~\cite{Rich1997,Sidner2005,Tambe2005a,Breazeal2003} and they
...
way. Finally, today only few systems~\cite{Fong_2006,Breazeal2003,Sisbot2008} take humans into
