...
%Mightability Analysis: Multi-State Visuo-Spatial perspective taking
...
\begin{table}[h]
    ...
    \hline
	 a location \par (\concept{Location})  & \textbf{is at} \par (\concept{isAt} $\equiv$ \concept{cyc:objectFoundInLocation})  &  a location \par (\concept{Location})  &  SPARK \\ 
	 &  $\rightarrow$ \textbf{is on} \par (\concept{isOn} $\equiv$ \concept{cyc:above\_Touching})  &  &  SPARK  \\ 
	 &  $\rightarrow$ \textbf{is in} \par (\concept{isIn})  &  &  SPARK  \\ 
	 &  $\rightarrow$ \textbf{is next to} \par (\concept{isNextTo})  &  &  SPARK  \\ 
	 a location \par (\concept{Location})  & \textbf{is above} \par (\concept{isAbove} $\equiv$ \concept{cyc:above-Generally})  &  a location \par (\concept{Location})  &  SPARK \par (inverse of isBelow)  \\ 
	 a location \par (\concept{Location})  & \textbf{is under} \par (\concept{isBelow})  &  a location \par (\concept{Location})  &  SPARK \par (inverse of isAbove)
	\end{tabular}
...
For instance, location of BottleA has no name $\rightarrow$ \texttt{bottleA
isAt loc12578}, with \texttt{loc12578} a random ID. 

If the position is actually completely unknown, \texttt{loc12578} has no
...
location. For instance: \texttt{loc12578 owl:differentFrom MyNiceTable}. 
...
\begin{table}[h]
	...
	\textbf{has relative position}
	\\ (\emph{hasRelativePosition})  & 
	...
	\\ (\emph{Location})  & 
	 ...
	 $\rightarrow$ \textbf{is behind}
	\\ (\emph{behind} $\equiv$ \emph{cyc:behind-Generally})  & 

	 & 

	 SPARK ? (inverse of inFrontOf)  \\ 


	 & 

	 $\rightarrow$ \textbf{is in front of}
	\\ (\emph{inFrontOf} $\equiv$ \emph{cyc:inFrontOf-Generally})  & 

	 & 

	 SPARK ? (inverse of behind)  \\ 


	 & 

	 $\rightarrow$ \textbf{is left of}
	\\ (\emph{leftOf})  & 

	 & 

	 SPARK ? (inverse of rightOf) \\ 


	 & 

	 $\rightarrow$ \textbf{is right of}
	\\ (\emph{rightOf})  & 

	 & 

	 SPARK ? (inverse of leftOf)  \\ 
...
\subsubsection{Agent attention}
\begin{table}[h]
	\centering
	\begin{tabular}{p{3cm}p{6cm}p{3cm}lp{3cm}}
		\textbf{Subject} & \textbf{Predicate} & \textbf{Object} & \emph{Computed by} & Details\\
		\hline
...
\emph{looksAt} is interpreted as an object \emph{being in the field of attention} of an agent.  \\ 
...
\\ (\emph{EnduringThing-Localized})  & 
 ...
 An object is \emph{seen} if it is visible for the agent without moving the head (ie, in \emph{field of view}).  \\ 


 a spatial enduring thing
\\ (\emph{EnduringThing-Localized})  & 

\textbf{is in field of view}
\\ (\emph{isInFieldOfView})  & 
...
\\ (\emph{xsd:boolean})  & 
 ...
 via inference: 
\\ myself sees * $\Rightarrow$ * isInFieldOfView true  \\ 
...
\textbf{points at}
\\ (\emph{pointsAt} $\equiv$ \emph{cyc:pointingToward})  & 
...
\\ (\emph{EnduringThing-Localized})  & 
 ...
 Via inference: 
\\ looksAt $\wedge$ pointsAt $\Rightarrow$ focusesOn

	\end{tabular}
	\caption{List of facts describing the attentional state of an agent.}
\end{table}



\subsubsection{Agent mightabilities}
...
\textbf{is visible (with possibly a head movement)}
\\ (\emph{isVisibleByMovingHead})  & 
...
\textbf{sees (with possible a head movement)}
\\ (\emph{seesWithHeadMovement})  & 

 a object
\\ (\emph{Object})   & 

 SPARK (via inference: 
\\ myself seesWithHeadMovement * $\Rightarrow$ * isVisibleByMovingHead true) \\ 


 a agent
\\ (\emph{Agent})  & 

\textbf{can not see (with possible a head movement)}
\\ (\emph{cannotSeeWithHeadMovement})  & 

 a object
\\ (\emph{Object})   & 

 SPARK (via inference: 
\\ myself cannotSeeWithHeadMovement * $\Rightarrow$ * isVisibleByMovingHead false) \\ 
...
\\ (\emph{Object})   & 
 ...
 SPARK (via inference: 
\\ myself reaches * $\Rightarrow$ * isReachable true) \\ 
...
\\ (\emph{Object})   & 
 ...
 SPARK (via inference: 
\\ myself cannotReach * $\Rightarrow$ * isReachable false)
...
\\ (\emph{hasIn\{Left|Right\}Hand})  & 

 a GraspableObject
\\ (\emph{GraspableObject})  & 
